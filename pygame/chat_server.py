...
# ==============================================================================
                ...
                my_send = text_proc(msg_s,from_name)
                self.indices[from_name].add_msg_and_index(my_send)
                
#                words = msg_s.split()
#                for wd in words:
#                    if wd not in self.indices[from_name].index.keys():
#                        self.indices[from_name].index[wd] = [my_send]
#                    else:
#                        self.indices[from_name].index[wd].append(my_send)
                
                
#                message = msg['message']
#                ctime = time.strftime('%d.%m.%y,%H:%M', time.localtime())
#                better_message = '(' + from_name + ')' + ctime + ' ' + message
#                self.indices[from_name].add_msg_and_index(better_message)
                ...
                for g in the_guys:
                    ...
                    self.indices[g].add_msg_and_index(my_send)
                    mysend(to_sock, json.dumps(
                            {"action":"exchange", "from":from_name, "message":msg_s}))
...
# ==============================================================================
                ...
                m = msg["target"]
                from_name = self.logged_sock2name[from_sock]
                history = self.indices[from_name]
                search_rslt = ''
                for x in history.search(m):
                    l,message = x
                    search_rslt += + message
            ...
            elif msg["action"] == "self_talking":
                mysend(from_sock, json.dumps(
                        {"action":"self_talking", "message":"pong blah blah"}))
                
#            elif msg["action"] == "script_killing":
                
##            elif msg["action"] == "playing":
##                mysend(from_sock, json.dumps(
##                        {"action":"playing", "message":"play with me!"}))
# ==============================================================================
